...
export class CableCar {
  ...
  _build() {
    ...
    STATION_TS.forEach((t, i) => {
      ...
      // Offset the pad to the outside of the spiral so it doesn't block the road.
      const out = 1 + (pp.width + 5) / Math.hypot(pp.x, pp.z);
    ...
    for (let i = 1; i < upLineCount; i++) {
      ...
      pylon.position.set(p.x, ground + h / 2, p.z);
      pylon.castShadow = true;
      scene.add(pylon);
      const arm = new THREE.Mesh(new THREE.BoxGeometry(3.2, 0.35, 0.35), pylonMat);
      arm.position.set(p.x, p.y - 0.25, p.z);
      scene.add(arm);
      const body = physics.world.createRigidBody(R.RigidBodyDesc.fixed().setTranslation(p.x, ground + h / 2, p.z));
  ...
  update() {
    ...
    let near = false;
    for (const g of this.gondolas) {
      const d = g.group.position.distanceTo(new THREE.Vector3(pp.x, pp.y, pp.z));
      if (d < 9) { near = true; break; }
    }
    if (near !== this._humOn) {
      this._humOn = near;
      sfx.cableHum(near);
